...
func TestParseFile_HappyPath_ValidJSONL(t *testing.T) {
	...
	recs, err := parseTranscriptFile(context.Background(), "testdata/transcripts/valid.jsonl", san)
	if err != nil {
...
func TestParseFile_MalformedLine_WarnsAndContinues(t *testing.T) {
	buf := captureSlog(t)
	san := newTestSanitizer(t)
	recs, err := parseTranscriptFile(context.Background(), "testdata/transcripts/malformed.jsonl", san)
	if err != nil {
		t.Fatalf("parseTranscriptFile: %v", err)
	}
...
func TestParseFile_PermissionDenied_ReturnsRuntimeError(t *testing.T) {
	...
	san := newTestSanitizer(t)
	_, err := parseTranscriptFile(context.Background(), path, san)
	if err == nil {
...
// TC-UC-11a: parseTranscriptDir on nonexistent dir → runtimeError.
func TestParseDir_MissingDir_ReturnsRuntimeError(t *testing.T) {
	san := newTestSanitizer(t)
	_, err := parseTranscriptDir(context.Background(), "testdata/no-such-dir", san)
	if err == nil {
		t.Fatalf("expected error, got nil")
	}
	var re *runtimeError
...
// TC-UC-11b: parseTranscriptDir on a dir with valid+malformed jsonl → returns
// valid records and warns about malformed lines.
func TestParseDir_MixedFiles_SkipsAndWarns(t *testing.T) {
	...
	buf := captureSlog(t)
	san := newTestSanitizer(t)
	recs, err := parseTranscriptDir(context.Background(), dir, san)
	if err != nil {
		t.Fatalf("parseTranscriptDir: %v", err)
	}
...
func TestParseFile_SecretsRedactedInBody(t *testing.T) {
	san := newTestSanitizer(t)
	recs, err := parseTranscriptFile(context.Background(), "testdata/transcripts/with-secrets.jsonl", san)
	if err != nil {
		t.Fatalf("parseTranscriptFile: %v", err)
	}
	if len(recs) != 1 {
...
func TestParseFile_ContextCancel(t *testing.T) {
	...
	cancel()
	_, err := parseTranscriptFile(ctx, "testdata/transcripts/valid.jsonl", san)
	if err == nil {
...
func TestParseFile_EmptyFile_Empty(t *testing.T) {
	...
	san := newTestSanitizer(t)
	recs, err := parseTranscriptFile(context.Background(), path, san)
	if err != nil {
		t.Fatalf("parseTranscriptFile empty: %v", err)
	}
	if len(recs) != 0 {
