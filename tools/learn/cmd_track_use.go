...
func runTrackUse(ctx context.Context, o trackUseOpts) error {
	...
	if o.DBPath == "" {
		return usagef("track-use: --db-path must be non-empty")
	}
	if len(o.Paths) == 0 {
	...
	logger := newLogger(o.Stderr, "track-use")

	st, openErr := openStore(o.DBPath)
	if openErr != nil {
		return runtimef("track-use: open store: %w", openErr)
	}
	...
	for _, path := range o.Paths {
		...
		if bumpErr != nil {
			return runtimef("track-use: increment skill_usage %q: %w", path, bumpErr)
		}
		if bumped {
			continue
		}
		...
		if bumpMemErr != nil {
			return runtimef("track-use: increment memory_usage %q: %w", path, bumpMemErr)
		}
		if bumpedMem {
			continue
		}
