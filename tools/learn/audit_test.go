...
func TestAppend_writesOneJSONLineAndCreatesDir(t *testing.T) {
	...
	if appendErr := appendAudit(dir, e); appendErr != nil {
		t.Fatalf("Append: %v", appendErr)
	}
	raw, readErr := os.ReadFile(filepath.Join(dir, auditFileName))
	if readErr != nil {
...
func TestAppend_appendsRatherThanTruncates(t *testing.T) {
	...
	for _, id := range []int64{1, 2, 3} {
		if appendErr := appendAudit(dir, auditEntry{DecisionID: id, Action: "applied", SourcePath: "x"}); appendErr != nil {
			t.Fatalf("Append: %v", appendErr)
		}
	}
	raw, readErr := os.ReadFile(filepath.Join(dir, auditFileName))
...
func TestRead_missingFileReturnsNil(t *testing.T) {
	t.Parallel()
	entries, readErr := readAudit(t.TempDir())
	if readErr != nil {
...
`
	if writeErr := os.WriteFile(filepath.Join(dir, auditFileName), []byte(mixed), 0o600); writeErr != nil {
		t.Fatalf("seed: %v", writeErr)
	}
	entries, readErr := readAudit(dir)
	if readErr != nil {
		t.Fatalf("Read: %v", readErr)
	}
	if len(entries) != 2 {
...
func TestFindByDecisionID_returnsLatestEntry(t *testing.T) {
	...
	got, findErr := findAuditByDecisionID(dir, 7)
	if findErr != nil {
		t.Fatalf("findAuditByDecisionID: %v", findErr)
	}
	if got == nil {
...
func TestFindByDecisionID_missingReturnsNil(t *testing.T) {
	t.Parallel()
	dir := filepath.Join(t.TempDir(), "learning")
	if appendErr := appendAudit(dir, auditEntry{DecisionID: 1, Action: "applied", SourcePath: "x"}); appendErr != nil {
		t.Fatalf("Append: %v", appendErr)
	}
	got, findErr := findAuditByDecisionID(dir, 99)
	if findErr != nil {
		t.Fatalf("findAuditByDecisionID: %v", findErr)
	}
	if got != nil {
...
func TestAppend_failsOnUnwritableDir(t *testing.T) {
	...
	err := appendAudit(dir, auditEntry{DecisionID: 1, Action: "applied", SourcePath: "x"})
	if err == nil {
		t.Fatalf("expected error, got nil")
	}
	var rt *runtimeError
	if !errors.As(err, &rt) {
		t.Errorf("want *runtimeError, got %T", err)
	}
}
