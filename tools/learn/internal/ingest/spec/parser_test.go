...
// newTestSanitizer builds the canonical sanitizer used across the parser tests.
// It pulls the default secret patterns so AKIA fixtures get redacted exactly
// as they would in production. A construction failure is fatal for the test
// process because every TC depends on a working sanitizer.
func newTestSanitizer(t *testing.T) *sanitize.Sanitizer {
	t.Helper()
	pats, buildErr := sanitize.BuiltinPatterns()
	if buildErr != nil {
		t.Fatalf("builtin sanitizer: %v", buildErr)
	}
...
func TestParse_MinimalDone(t *testing.T) {
	...
	records, parseErr := spec.Parse(context.Background(), fixture("minimal-done.md"), san)
	if parseErr != nil {
...
func TestParse_DraftSkipsTransitionAndTasks(t *testing.T) {
	...
	san := newTestSanitizer(t)
	records, parseErr := spec.Parse(context.Background(), fixture("draft.md"), san)
	if parseErr != nil {
		t.Fatalf("Parse: %v", parseErr)
	}
...
func TestParse_EmptyFile(t *testing.T) {
	...
	san := newTestSanitizer(t)
	records, parseErr := spec.Parse(context.Background(), fixture("empty.md"), san)
	if parseErr != nil {
		t.Fatalf("Parse(empty): %v", parseErr)
	}
	if len(records) != 0 {
...
`
	...
	san := newTestSanitizer(t)
	records, parseErr := spec.Parse(context.Background(), path, san)
	if parseErr != nil {
		t.Fatalf("Parse: %v", parseErr)
	}
	for _, r := range records {
...
func TestParse_SanitizesAWSKey_TC_UC_08(t *testing.T) {
	...
	san := newTestSanitizer(t)
	records, parseErr := spec.Parse(context.Background(), fixture("with-aws-key.md"), san)
	if parseErr != nil {
		t.Fatalf("Parse: %v", parseErr)
	}
	for _, r := range records {
...
func TestParse_FileNotFound_ReturnsRuntimeError(t *testing.T) {
	...
	san := newTestSanitizer(t)
	_, parseErr := spec.Parse(context.Background(), fixture("does-not-exist.md"), san)
	if parseErr == nil {
		t.Fatal("Parse: expected error for missing file")
	}
...
// writeFile is a tiny helper used by tests that synthesize a fixture on disk
// instead of placing it under testdata/specs/.
