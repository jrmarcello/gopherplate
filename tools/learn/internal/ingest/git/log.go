// Package git harvests Records from a local git repository's commit history.
// It is one of the four ingest sources of the learning-loop harness (REQ-2 of
// .specs/learning-loop-harness.md). Each commit becomes a single Record whose
// Tokens slice encodes the conventional-commit type/scope plus the unique
// directory prefixes of the changed files — downstream pattern miners consume
// Tokens to build ngrams without re-parsing the raw subject line.
//
...
// byte-identical Records. The sanitize.Sanitizer is applied to the commit
// body before it leaves this package, never after.
package git
...
// MaxDirsPerCommit caps the number of unique directory-prefix tokens emitted
...
// Record is the harvested shape of a single commit. Fields are intentionally
// duplicated across the four ingest packages (git, prompts, ci, eval) so each
// package can be tested in isolation; the contract is locked by the spec
// (REQ-2..REQ-5) and any divergence between packages is a spec violation.
type Record struct {
	Source string    // commit sha (full 40-char hex)
	Kind   string    // always "commit" for this package
	Tokens []string  // [type, scope, dir1, dir2, ...] — see package doc
	SeenAt time.Time // author timestamp
	Body   string    // sanitized subject + body
}

// Options tune the git log invocation. The zero value is valid: no since
...
// ParseLog runs `git log` against repoDir and returns one Record per commit.
// Binary diffs are detected via --numstat's "-\t-" marker and the entire
// commit is skipped with a structured warn on the package logger; this is
// TC-UC-11a from the spec. The Body field is sanitized via san before
// being returned, so callers can safely persist the Record without
// re-running redaction.
//
...
// *learnerr.RuntimeError so the CLI exits with code 2 per the REQ-22
// contract.
func ParseLog(ctx context.Context, repoDir string, opts Options, san *sanitize.Sanitizer) ([]Record, error) {
	if san == nil {
		return nil, learnerr.Runtimef("git.ParseLog: sanitizer is required")
	}
	if repoDir == "" {
		wd, wdErr := os.Getwd()
		if wdErr != nil {
			return nil, learnerr.Runtimef("git.ParseLog: resolve cwd: %w", wdErr)
		}
		repoDir = wd
	}
	...
	if runErr != nil {
		...
		if errors.As(runErr, &exitErr) {
			return nil, learnerr.Runtimef("git.ParseLog: git log exited %d: %s: %w", exitErr.ExitCode(), strings.TrimSpace(stderrText), runErr)
		}
		return nil, learnerr.Runtimef("git.ParseLog: run git: %w", runErr)
	}
	...
	return parseEntries(string(out), san)
}

// parseEntries is the pure parser exposed to unit tests. It accepts the raw
// stdout of the git-log command (see prettyFormat) and returns one Record
// per textual entry. Binary-diff commits are dropped with a warn on the
// package logger. The function never touches the filesystem or the process
// environment — every external concern lives in ParseLog.
func parseEntries(raw string, san *sanitize.Sanitizer) ([]Record, error) {
	...
	flush := func() {
		...
		rec, binary, parseErr := buildRecord(text, numstat, san)
		if parseErr != nil {
			log.Printf("git.parseEntries: skipping malformed entry: %v", parseErr)
			return
		}
		if binary {
			log.Printf("git.parseEntries: skipping commit %s (binary diff)", rec.Source)
			return
		}
	...
	if scanErr := scanner.Err(); scanErr != nil {
		return nil, learnerr.Runtimef("git.parseEntries: scan output: %w", scanErr)
	}
	flush()
...
// buildRecord turns one header block + its numstat lines into a Record.
...
func buildRecord(header string, numstat []string, san *sanitize.Sanitizer) (Record, bool, error) {
	...
	if len(fields) < 4 {
		return Record{}, false, learnerr.Runtimef("expected 4 header fields, got %d: %q", len(fields), headerText)
	}
	...
	if dateErr != nil {
		return Record{}, false, learnerr.Runtimef("parse date %q: %w", dateStr, dateErr)
	}

	files, binary := parseNumstat(numstat)
	if binary {
		return Record{Source: sha}, true, nil
	}
...
// alphabetically and capped at MaxDirsPerCommit. Files at the repo root
...
func dirTokens(files []string) []string {
	...
	sort.Strings(dirs)
	if len(dirs) > MaxDirsPerCommit {
		dirs = dirs[:MaxDirsPerCommit]
	}
	return dirs
}
...
// itoa is a tiny helper that avoids pulling in strconv for the single
// integer-to-string conversion ParseLog needs. Keeping the import surface
// small makes the package easier to audit.
