...
// delegating to internal/recall.Recall and formatting the result either as
...
import (
	...
	"github.com/jrmarcello/gopherplate/tools/learn/internal/recall"
...
type recallOpts struct {
	...
	// Defaults to time.Now when nil. Forwarded to recall.Options.
	Now func() time.Time
}
...
and empty result sets produce no output and exit 0.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if loadErr != nil {
				return learnerr.Runtimef("recall: load config: %w", loadErr)
			}
...
// recall.Recall, and formats the output per o.Format.
func runRecall(ctx context.Context, o recallOpts) error {
	...
	if o.TopK < 1 {
		return learnerr.Usagef("recall: --top-k must be >= 1, got %d", o.TopK)
	}
	if o.MaxTokens < 1 {
		return learnerr.Usagef("recall: --max-tokens must be >= 1, got %d", o.MaxTokens)
	}
	if o.MinScore < 0 {
		return learnerr.Usagef("recall: --min-score must be >= 0, got %v", o.MinScore)
	}
	if o.Since < 0 {
		return learnerr.Usagef("recall: --since must be >= 0, got %s", o.Since)
	}
	switch o.KindFilter {
	case "", recall.KindSkill, recall.KindMemory, recall.KindPattern:
		// ok
	default:
		return learnerr.Usagef(
	...
	case "", "system-reminder", "json":
		// ok
	default:
		return learnerr.Usagef("recall: --format=%q invalid (valid: system-reminder, json)", o.Format)
	}
	...
	if o.DBPath == "" {
		return learnerr.Usagef("recall: --db-path must be non-empty")
	}
	st, openErr := store.Open(o.DBPath)
	if openErr != nil {
		return learnerr.Runtimef("recall: open store: %w", openErr)
	}
	defer func() { _ = st.Close() }()

	matches, recErr := recall.Recall(ctx, st.DB(), recall.Options{
		Prompt:     o.Prompt,
		TopK:       o.TopK,
...
func writeJSON(w io.Writer, matches []recall.Match) error {
	body, marshalErr := json.MarshalIndent(matches, "", "  ")
	if marshalErr != nil {
		return learnerr.Runtimef("recall: marshal json: %w", marshalErr)
	}
	body = append(body, '\n')
	if _, writeErr := w.Write(body); writeErr != nil {
		return learnerr.Runtimef("recall: write stdout: %w", writeErr)
	}
	return nil
}
...
func writeSystemReminder(w io.Writer, matches []recall.Match) error {
