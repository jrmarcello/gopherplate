...
//
// Config, when non-nil, short-circuits config.Load — useful for tests that
...
by the nudge subcommand, not here.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if specPath == "" {
				return learnerr.Usagef("complete-task: --spec is required")
			}
			if sessionID == "" {
				return learnerr.Usagef("complete-task: --session is required")
			}
	...
	// error becomes a *learnerr.UsageError (exit 1) instead of cobra's default
...
//
// All IO failures are wrapped as *learnerr.RuntimeError so the dispatcher
// exits with code 2.
func runCompleteTask(ctx context.Context, o completeOpts) error {
	if o.SpecPath == "" {
		return learnerr.Usagef("complete-task: spec path must be non-empty")
	}
	if o.SessionID == "" {
		return learnerr.Usagef("complete-task: session id must be non-empty")
	}
	...
	if cfg == nil {
		...
		if loadErr != nil {
			return learnerr.Runtimef("complete-task: load config: %w", loadErr)
		}
		cfg = loaded
	}
	...
	if dbPath == "" {
		if o.LearningDir == "" {
			return learnerr.Usagef("complete-task: --db-path or --dir must be set")
		}
	...
	if openErr != nil {
		return learnerr.Runtimef("complete-task: open store: %w", openErr)
	}
	...
	if insertErr := st.InsertEvent(ctx, ev); insertErr != nil {
		return learnerr.Runtimef("complete-task: insert event: %w", insertErr)
	}

	newCounter, incrementErr := st.IncrementNudgeCounter(ctx)
	if incrementErr != nil {
		return learnerr.Runtimef("complete-task: increment nudge counter: %w", incrementErr)
	}

	if newCounter >= cfg.NudgeThreshold {
		if _, writeErr := fmt.Fprintf(stderr, "LEARN_NUDGE_DUE=true counter=%d\n", newCounter); writeErr != nil {
			return learnerr.Runtimef("complete-task: emit nudge signal: %w", writeErr)
		}
	}
	return nil
}
