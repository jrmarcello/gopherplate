...
func runSmoke(ctx context.Context, o smokeOpts) error {
	...
	if mkErr != nil {
		return learnerr.Runtimef("smoke: mkdir temp: %w", mkErr)
	}
	if !o.KeepArtifacts {
	...
	if initErr := runInit(initOpts{
		...
		return learnerr.Runtimef("smoke: init: %w", initErr)
	}
...
`, i, specsDir, specsDir, i, i)
		...
		if ctErr := runCompleteTask(ctx, ctOpts); ctErr != nil {
			return learnerr.Runtimef("smoke: complete-task %d: %w", i, ctErr)
		}
	}
	...
	if openErr != nil {
		return learnerr.Runtimef("smoke: reopen store: %w", openErr)
	}
	defer func() { _ = st.Close() }()
	var eventsCount int
	if countErr := st.DB().QueryRowContext(ctx, "SELECT count(*) FROM events").Scan(&eventsCount); countErr != nil {
		return learnerr.Runtimef("smoke: count events: %w", countErr)
	}
	if eventsCount != 3 {
		return learnerr.Runtimef("smoke: expected 3 events, got %d", eventsCount)
	}
	if closeErr := st.Close(); closeErr != nil {
		return learnerr.Runtimef("smoke: close store before extract: %w", closeErr)
	}
	...
	if loadErr != nil {
		return learnerr.Runtimef("smoke: load config: %w", loadErr)
	}
	...
	if extErr := runExtract(ctx, extOpts); extErr != nil {
		return learnerr.Runtimef("smoke: extract: %w", extErr)
	}
	...
	if _, statErr := os.Stat(candidatesPath); statErr != nil {
		return learnerr.Runtimef("smoke: candidates.jsonl missing: %w", statErr)
	}
...
`
	...
	if rxErr := runReindex(ctx, rxOpts); rxErr != nil {
		return learnerr.Runtimef("smoke: reindex: %w", rxErr)
	}
	...
	if rcErr := runRecall(ctx, rcOpts); rcErr != nil {
		return learnerr.Runtimef("smoke: recall: %w", rcErr)
	}
	if !strings.Contains(recallBuf.String(), "smoke-skill") {
		return learnerr.Runtimef("smoke: recall output missing seeded skill — got: %s",
	...
	if stErr := runStats(ctx, stOpts); stErr != nil {
		return learnerr.Runtimef("smoke: stats: %w", stErr)
	}
