...
func runRefineApply(ctx context.Context, o refineApplyOpts) error {
	...
	if o.CandidateSignature == "" {
		return learnerr.Usagef("refine-apply: --candidate-signature is required")
	}
	if o.TargetPath == "" {
		return learnerr.Usagef("refine-apply: --target-path is required")
	}
	if o.DBPath == "" {
		return learnerr.Usagef("refine-apply: --db-path is required")
	}
	if o.MergedInto == "" {
	...
	if o.DiffFile != "" {
		...
		if readErr != nil {
			return learnerr.Runtimef("refine-apply: read --diff-file %q: %w", o.DiffFile, readErr)
		}
	...
	if openErr != nil {
		return learnerr.Runtimef("refine-apply: open store: %w", openErr)
	}
...
RETURNING id`
	...
	if scanErr := st.DB().QueryRowContext(ctx, insertStmt,
		...
		return learnerr.Runtimef("refine-apply: insert decision: %w", scanErr)
	}
	...
	if _, execErr := st.DB().ExecContext(ctx, updateStmt, now.Format(time.RFC3339Nano), id); execErr != nil {
		return learnerr.Runtimef("refine-apply: mark decision %d applied (file already moved to %q): %w", id, deprecatedPath, execErr)
	}
