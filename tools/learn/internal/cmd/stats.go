...
downstream consumers (CI checks, dashboards) can rely on it.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if format != "" && format != "json" {
				return learnerr.Usagef("stats: --format=%q not supported (only json)", format)
			}
...
// query is a *learnerr.RuntimeError (exit 2). The function never panics on
...
func runStats(ctx context.Context, o statsOpts) error {
	...
	logger := logging.New(o.Stderr, "stats")

	if o.DBPath == "" {
		return learnerr.Usagef("stats: --db-path must be non-empty")
	}
	...
	if _, statErr := os.Stat(filepath.Dir(o.DBPath)); statErr != nil {
		...
		return learnerr.Runtimef("stats: stat %q: %w", filepath.Dir(o.DBPath), statErr)
	}

	st, openErr := store.Open(o.DBPath)
	if openErr != nil {
		// store.Open already returns *RuntimeError; preserve it via %w.
	...
	if marshalErr != nil {
		...
		return learnerr.Runtimef("stats: marshal output: %w", marshalErr)
	}
	body = append(body, '\n')
	if _, writeErr := o.Stdout.Write(body); writeErr != nil {
		logger.Error("write stdout failed", "err", writeErr.Error())
		return learnerr.Runtimef("stats: write stdout: %w", writeErr)
	}
...
func collectStats(ctx context.Context, db *sql.DB, dbPath string) (statsOutput, error) {
	...
	for _, t := range tables {
		if countErr := db.QueryRowContext(ctx,
			...
			return out, learnerr.Runtimef("stats: count %s: %w", t.name, countErr)
		}
	}
	...
	if scanErr := db.QueryRowContext(ctx,
		...
		return out, learnerr.Runtimef("stats: max(candidates.created_at): %w", scanErr)
	}
	...
	if scanErr := db.QueryRowContext(ctx,
		...
		if errors.Is(scanErr, sql.ErrNoRows) {
			...
			return out, learnerr.Runtimef("stats: read nudge_state: %w", scanErr)
		}
	}
	if lastNudge.Valid {
	...
	if topErr != nil {
		return out, learnerr.Runtimef("stats: top skills: %w", topErr)
	}
	...
	if bottomErr != nil {
		return out, learnerr.Runtimef("stats: bottom skills: %w", bottomErr)
	}
	...
	if info, statErr := os.Stat(dbPath); statErr == nil {
		...
		return out, learnerr.Runtimef("stats: stat db file: %w", statErr)
	}
	return out, nil
}
