...
// `sanitize.Sanitizer` before being written to stdout. The agent that
...
func runAuditSkillsPrep(o auditSkillsPrepOpts) error {
	if o.SkillsDir == "" {
		return learnerr.Usagef("audit-skills-prep: --skills-dir must be non-empty")
	}
	if o.MaxBodyChars <= 0 {
	...
	san, sanErr := sanitize.NewFromConfig(config.DefaultSecretPatterns)
	if sanErr != nil {
		return learnerr.Runtimef("audit-skills-prep: build sanitizer: %w", sanErr)
	}
	...
	if encErr := enc.Encode(&out); encErr != nil {
		return learnerr.Runtimef("audit-skills-prep: encode output: %w", encErr)
	}
	return nil
}
...
func summariseSkillFile(path string, maxBodyChars int, san *sanitize.Sanitizer) (skillSummary, error) {
	...
	if splitErr != nil {
		return skillSummary{}, learnerr.Usagef("audit-skills-prep: %s: %w", path, splitErr)
	}
