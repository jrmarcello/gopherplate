...
func runRecordDecision(ctx context.Context, o recordDecisionOpts) error {
	...
	if o.CandidateSignature == "" {
		return learnerr.Usagef("record-decision: --candidate-signature is required")
	}
	if _, ok := validRecordActions[o.Action]; !ok {
		return learnerr.Usagef("record-decision: invalid --action %q (want one of new-skill|new-memory|update|discard|pending-approval)", o.Action)
	}
	if o.DBPath == "" {
		return learnerr.Usagef("record-decision: --db-path must be non-empty")
	}
	...
	if o.Action != "discard" && o.TargetPath == "" {
		return learnerr.Usagef("record-decision: --target-path is required for action %q", o.Action)
	}
	...
	if o.DiffFile != "" {
		...
		if readErr != nil {
			return learnerr.Runtimef("record-decision: read --diff-file %q: %w", o.DiffFile, readErr)
		}
	...
	case "update":
		...
		if captureErr != nil && !errors.Is(captureErr, os.ErrNotExist) {
			return learnerr.Runtimef("record-decision: capture pre-update %q: %w", o.TargetPath, captureErr)
		}
	...
	if openErr != nil {
		...
		return learnerr.Runtimef("record-decision: open store: %w", openErr)
	}
...
VALUES (?, ?, ?, ?, ?, ?)`
	if _, execErr := st.DB().ExecContext(ctx, insertDecision,
		...
		return learnerr.Runtimef("record-decision: insert decision: %w", execErr)
	}
	return nil
}
...
func writeNewLearningFile(o recordDecisionOpts) error {
	if o.BodyFile == "" {
		return learnerr.Usagef("record-decision: --body-file is required for action %q", o.Action)
	}
	body, readErr := os.ReadFile(o.BodyFile)
	if readErr != nil {
		return learnerr.Runtimef("record-decision: read --body-file %q: %w", o.BodyFile, readErr)
	}
	if _, statErr := os.Stat(o.TargetPath); statErr == nil {
		return learnerr.Usagef("record-decision: target %q already exists (use --action=update)", o.TargetPath)
	} else if !errors.Is(statErr, os.ErrNotExist) {
		return learnerr.Runtimef("record-decision: stat %q: %w", o.TargetPath, statErr)
	}
...
func applyUpdateToLearningFile(o recordDecisionOpts) error {
	...
	if statErr != nil {
		return learnerr.Runtimef("record-decision: read target %q: %w", o.TargetPath, statErr)
	}

	fmMap, bodyText, splitErr := splitFrontmatterRaw(existing)
	if splitErr != nil {
		return learnerr.Usagef("record-decision: parse target %q: %w", o.TargetPath, splitErr)
	}
	...
	default:
		return learnerr.Usagef("record-decision: invalid --mode %q (want append|replace)", mode)
	}
