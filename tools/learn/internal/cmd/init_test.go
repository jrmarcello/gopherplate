...
func TestRunInit_freshDirectory(t *testing.T) {
	...
	if openErr != nil {
		t.Fatalf("store.Open after init: %v", openErr)
	}
...
func TestRunInit_idempotentPreservesData(t *testing.T) {
	...
	st2, openErr2 := store.Open(dbPath)
	if openErr2 != nil {
		t.Fatalf("store.Open after second init: %v", openErr2)
	}
...
// *learnerr.RuntimeError. Skipped on Windows and when running as root (chmod
...
func TestRunInit_unwritableParent_returnsRuntimeError(t *testing.T) {
	...
	if !errors.As(runErr, &re) {
		t.Errorf("expected *learnerr.RuntimeError, got %T: %v", runErr, runErr)
	}
}
