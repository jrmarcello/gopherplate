...
func runReindex(ctx context.Context, o reindexOpts) error {
	if o.DBPath == "" {
		return learnerr.Usagef("reindex: --db-path must be non-empty")
	}
	if o.Now == nil {
	...
	cfg, loadErr := config.Load(o.ConfigPath)
	if loadErr != nil {
		return learnerr.Runtimef("reindex: load config: %w", loadErr)
	}
	san, sanErr := sanitize.NewFromConfig(cfg.SecretPatterns)
	if sanErr != nil {
		return learnerr.Runtimef("reindex: build sanitizer: %w", sanErr)
	}

	st, openErr := store.Open(o.DBPath)
	if openErr != nil {
		return learnerr.Runtimef("reindex: open store: %w", openErr)
	}
...
func reindexSinglePath(ctx context.Context, st *store.Store, san *sanitize.Sanitizer, o reindexOpts) error {
	info, statErr := os.Stat(o.SinglePath)
	if statErr != nil {
		if errors.Is(statErr, fs.ErrNotExist) {
			return learnerr.Usagef("reindex: --path %q: file does not exist", o.SinglePath)
		}
		return learnerr.Runtimef("reindex: stat %q: %w", o.SinglePath, statErr)
	}
	if info.IsDir() {
		return learnerr.Usagef("reindex: --path %q: is a directory, expected a file", o.SinglePath)
	}
	...
	default:
		return learnerr.Usagef("reindex: --path %q: unsupported file (expected SKILL.md or *.md)", o.SinglePath)
	}
}
...
func scanSkillFiles(skillsDir string, sinceMtime time.Duration, now time.Time) ([]string, error) {
	if _, statErr := os.Stat(skillsDir); statErr != nil {
		...
		return nil, learnerr.Runtimef("reindex: stat skills dir %q: %w", skillsDir, statErr)
	}
	...
	if walkErr != nil {
		return nil, learnerr.Runtimef("reindex: walk skills %q: %w", skillsDir, walkErr)
	}
	sort.Strings(out)
	return out, nil
}
...
func scanMemoryFiles(memoryDir string, sinceMtime time.Duration, now time.Time) ([]string, error) {
	...
	if readErr != nil {
		...
		return nil, learnerr.Runtimef("reindex: readdir memory %q: %w", memoryDir, readErr)
	}
	var out []string
...
func upsertSkill(ctx context.Context, st *store.Store, san *sanitize.Sanitizer, path string, now time.Time) error {
	...
	if upsertErr := st.UpsertSkillIndex(ctx, entry); upsertErr != nil {
		return learnerr.Runtimef("reindex: upsert skill %q: %w", path, upsertErr)
	}
	return nil
}
...
func upsertMemory(ctx context.Context, st *store.Store, san *sanitize.Sanitizer, path string, deprecatedRoots []string, now time.Time) error {
	rec, parseErr := memory.ParseFile(ctx, path, deprecatedRoots, san)
	if parseErr != nil {
		return learnerr.Runtimef("reindex: parse memory %q: %w", path, parseErr)
	}
	...
	if marshalErr != nil {
		return learnerr.Runtimef("reindex: marshal frontmatter %q: %w", path, marshalErr)
	}
	...
	if upsertErr := st.UpsertMemoryIndex(ctx, entry); upsertErr != nil {
		return learnerr.Runtimef("reindex: upsert memory %q: %w", path, upsertErr)
	}
	return nil
}
...
func parseSkillFile(path string, san *sanitize.Sanitizer) (*parsedSkill, error) {
	if san == nil {
	...
	if splitErr != nil {
		return nil, learnerr.Usagef("reindex: parse %q: %w", path, splitErr)
	}
	...
	if marshalErr != nil {
		return nil, learnerr.Runtimef("reindex: marshal frontmatter %q: %w", path, marshalErr)
	}
...
func pruneOrphanSkills(ctx context.Context, st *store.Store, skillsDir string) error {
	...
	for _, p := range paths {
		...
		if delErr := st.DeleteOrphanSkill(ctx, p); delErr != nil {
			return learnerr.Runtimef("reindex: delete orphan skill %q: %w", p, delErr)
		}
	}
	return nil
}
...
func pruneOrphanMemory(ctx context.Context, st *store.Store, memoryDir string) error {
	...
	for _, p := range paths {
		...
		if delErr := st.DeleteOrphanMemory(ctx, p); delErr != nil {
			return learnerr.Runtimef("reindex: delete orphan memory %q: %w", p, delErr)
		}
	}
	return nil
}
