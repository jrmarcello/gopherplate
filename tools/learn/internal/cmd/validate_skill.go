...
func runValidateSkill(o validateSkillOpts) error {
	if o.Path == "" {
		return learnerr.Usagef("validate-skill: path is required")
	}
	raw, readErr := os.ReadFile(o.Path)
	if readErr != nil {
		return learnerr.Runtimef("validate-skill: read %q: %w", o.Path, readErr)
	}
	fmMap, parseErr := parseFrontmatterOnly(raw)
	if parseErr != nil {
		return learnerr.Usagef("validate-skill: %s: %w", o.Path, parseErr)
	}
	...
	if len(missing) > 0 {
		return learnerr.Usagef("validate-skill: %s: missing required frontmatter field(s): %s", o.Path, strings.Join(missing, ", "))
	}
	return nil
}
