...
func seedSkillIndex(t *testing.T, ctx context.Context, st *store.Store, path, title, body, tags string) {
	t.Helper()
...
func TestRunSimilar_threshold_belowExcluded(t *testing.T) {
	...
	mockQuery := func(ctx context.Context, db *sql.DB, opts similar.QueryOpts) ([]similar.Match, error) {
		...
		if opts.MinScore > 0.59 {
			...
			return []similar.Match{}, nil
		}
		return []similar.Match{
			{Index: similar.IndexSkill, Path: "skills/beta/SKILL.md", Score: 0.59},
		}, nil
	}
...
func TestRunSimilar_threshold_inclusiveAtBoundary(t *testing.T) {
	...
	mockQuery := func(ctx context.Context, db *sql.DB, opts similar.QueryOpts) ([]similar.Match, error) {
		// similar.Query semantics: returns matches with score >= MinScore.
		if 0.60 < opts.MinScore {
			return []similar.Match{}, nil
		}
		return []similar.Match{
			{Index: similar.IndexSkill, Path: "skills/beta/SKILL.md", Score: 0.60},
		}, nil
	}
...
func TestRunSimilar_threshold_aboveIncluded(t *testing.T) {
	...
	mockQuery := func(ctx context.Context, db *sql.DB, opts similar.QueryOpts) ([]similar.Match, error) {
		...
		return []similar.Match{
			{Index: similar.IndexSkill, Path: "skills/beta/SKILL.md", Score: 0.61},
		}, nil
	}
...
func TestRunSimilar_missingSkillFlag(t *testing.T) {
	...
	if !errors.As(runErr, &ue) {
		t.Errorf("expected *learnerr.UsageError, got %T: %v", runErr, runErr)
	}
}
