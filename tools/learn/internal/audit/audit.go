// Package audit appends one JSONL line per applied / rolled-back learning-loop
...
// FileName is the canonical basename inside the learning directory.
...
// Path resolves the canonical audit file path inside learningDir.
func Path(learningDir string) string {
	return filepath.Join(learningDir, FileName)
}

// Append serializes e to JSON and appends a single line to <learningDir>/audit.jsonl.
// The directory is created with 0o700 if missing (matching init.go's learning-dir
// policy). Failures wrap as *learnerr.RuntimeError; callers may choose to log
// rather than bubble (the audit log is observability, not the source of truth).
func Append(learningDir string, e Entry) error {
	if e.Timestamp == "" {
		e.Timestamp = time.Now().UTC().Format(time.RFC3339Nano)
	}
	line, marshalErr := json.Marshal(&e)
	if marshalErr != nil {
		return learnerr.Runtimef("audit: marshal entry: %w", marshalErr)
	}
	...
	if mkErr := os.MkdirAll(learningDir, 0o700); mkErr != nil {
		return learnerr.Runtimef("audit: create learning dir %q: %w", learningDir, mkErr)
	}
	path := Path(learningDir)
	f, openErr := os.OpenFile(path, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0o600) //nolint:gosec // operator-owned learning dir
	if openErr != nil {
		return learnerr.Runtimef("audit: open %q: %w", path, openErr)
	}
	defer func() { _ = f.Close() }()
	if _, writeErr := f.Write(append(line, '\n')); writeErr != nil {
		return learnerr.Runtimef("audit: write %q: %w", path, writeErr)
	}
	return nil
}

// Read returns every Entry in audit.jsonl. Missing file returns nil, nil
...
func Read(learningDir string) ([]Entry, error) {
	path := Path(learningDir)
	...
	if openErr != nil {
		...
		return nil, learnerr.Runtimef("audit: open %q: %w", path, openErr)
	}
	defer func() { _ = f.Close() }()
	...
	if scanErr := scanner.Err(); scanErr != nil {
		return out, learnerr.Runtimef("audit: scan %q: %w", path, scanErr)
	}
	return out, nil
}

// FindByDecisionID returns the LATEST entry for the given decision id (a
...
func FindByDecisionID(learningDir string, id int64) (*Entry, error) {
	entries, readErr := Read(learningDir)
	if readErr != nil {
		return nil, readErr
	}
	var found *Entry
...
// formatTimestamp is exported for tests that want deterministic timestamps.
// Production callers leave Entry.Timestamp empty and rely on Append's
// time.Now() default.
func FormatTimestamp(t time.Time) string {
	return t.UTC().Format(time.RFC3339Nano)
}

// Ensure fmt is retained even if no callers use it inline; keeps gosec
// happy if a future refactor adds Errorf-style wrapping.
var _ = fmt.Errorf
