...
// newGitTestSanitizer returns a sanitizer with the canonical aws_key rule used
// by the git redaction TCs (REQ-4). Built directly here so tests don't depend
// on the project's config file existing on disk. Distinct from the shared
// newTestSanitizer because git tests use a single-pattern sanitizer (the
// existing test wanted minimal scope) rather than the full builtin set.
func newGitTestSanitizer(t *testing.T) *sanitizer {
	t.Helper()
	specs := []secretPattern{
		{Kind: "aws_key", Pattern: `AKIA[0-9A-Z]{16}`},
	}
	san, err := newSanitizerFromConfig(specs)
	if err != nil {
...
// --- parseGitEntries unit tests (pure function, no git needed) ---

func TestParseEntries_Happy_ConventionalCommit(t *testing.T) {
	san := newGitTestSanitizer(t)
	raw := joinEntries(
	...
	records, err := parseGitEntries(raw, san)
	if err != nil {
		t.Fatalf("parseGitEntries: %v", err)
	}
	if len(records) != 1 {
...
func TestParseEntries_MaxDirsCap(t *testing.T) {
	...
	if len(dirTokens) != maxGitDirsPerCommit {
		t.Errorf("dir tokens: want %d, got %d (%v)", maxGitDirsPerCommit, len(dirTokens), dirTokens)
	}
	// alphabetical
...
func TestParseEntries_EmptyInput(t *testing.T) {
	san := newGitTestSanitizer(t)
	records, err := parseGitEntries("", san)
	if err != nil {
		t.Fatalf("parseGitEntries empty: %v", err)
	}
	if len(records) != 0 {
...
// --- parseGitLog integration test (requires `git` on PATH) ---
...
func TestParseLog_HappyPath_IntegratesWithGit(t *testing.T) {
	...
	san := newGitTestSanitizer(t)
	records, err := parseGitLog(context.Background(), repo, gitOptions{}, san)
	if err != nil {
...
func TestParseLog_EmptyRepo(t *testing.T) {
	...
	records, err := parseGitLog(context.Background(), repo, gitOptions{}, san)
	if err != nil {
		t.Fatalf("parseGitLog empty: %v", err)
	}
	if len(records) != 0 {
		t.Errorf("want 0 records, got %d", len(records))
	}
...
func TestParseLog_SinceFilter(t *testing.T) {
	...
	san := newGitTestSanitizer(t)
	records, err := parseGitLog(context.Background(), repo, gitOptions{Since: "1 day ago"}, san)
	if err != nil {
		t.Fatalf("parseGitLog: %v", err)
	}
	if len(records) != 1 {
...
func TestParseLog_RuntimeErrorOnBadRepo(t *testing.T) {
	...
	_, err := parseGitLog(context.Background(), dir, gitOptions{}, san)
	if err == nil {
		t.Fatal("expected error from non-git dir")
	}
	var re *runtimeError
	if !errors.As(err, &re) {
		t.Errorf("want *runtimeError, got %T: %v", err, err)
	}
}
