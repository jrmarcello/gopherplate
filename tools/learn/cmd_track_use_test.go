...
func seedTrackStore(t *testing.T) (string, *sqliteStore) {
	t.Helper()
	dir := t.TempDir()
	...
	if upsertErr := st.UpsertSkillIndex(ctx, skillIndexEntry{
	...
	if upsertErr := st.UpsertMemoryIndex(ctx, memoryIndexEntry{
		Path:            "memory/b.md",
		Title:           "b",
		Body:            "body b",
		FrontmatterJSON: "{}",
