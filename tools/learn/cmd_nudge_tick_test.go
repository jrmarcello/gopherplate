...
func seedNudgeStore(t *testing.T) (string, *sqliteStore) {
	t.Helper()
	dir := t.TempDir()
...
func insertSkillWithUsage(t *testing.T, st *sqliteStore, path, indexedAt string, lastUsedAt *string, usageCount int) {
	t.Helper()
	ctx := context.Background()
	if _, execErr := st.DB().ExecContext(ctx,
...
func parseTickJSON(t *testing.T, dbPath string, cfg *loopConfig, now time.Time) nudgeTickOutput {
	t.Helper()
	var stdout, stderr bytes.Buffer
