...
func runRollback(ctx context.Context, o rollbackOpts) error {
	...
	if o.ID <= 0 {
		return usagef("rollback: --id must be > 0")
	}
	if o.DBPath == "" {
		return usagef("rollback: --db-path is required")
	}

	learningDir := learningDirFromDB(o.DBPath)
	entry, findErr := findAuditByDecisionID(learningDir, o.ID)
	if findErr != nil {
		return findErr
	}
	if entry == nil {
		return usagef("rollback: no audit entry for decision %d", o.ID)
	}
	if entry.Action == "rolled-back" {
		return usagef("rollback: decision %d is already rolled back (audit entry action=rolled-back)", o.ID)
	}
	if entry.DeprecatedPath == "" || entry.SourcePath == "" {
		return usagef("rollback: decision %d has incomplete audit entry (deprecated_path=%q source_path=%q)",
			o.ID, entry.DeprecatedPath, entry.SourcePath)
	}

	if _, statErr := os.Stat(entry.DeprecatedPath); errors.Is(statErr, os.ErrNotExist) {
		return usagef("rollback: deprecated file %q no longer exists on disk", entry.DeprecatedPath)
	} else if statErr != nil {
		return runtimef("rollback: stat deprecated file %q: %w", entry.DeprecatedPath, statErr)
	}
	if _, statErr := os.Stat(entry.SourcePath); statErr == nil {
		return usagef("rollback: canonical path %q is occupied (another file landed there since apply); resolve manually first", entry.SourcePath)
	} else if !errors.Is(statErr, os.ErrNotExist) {
		return runtimef("rollback: stat canonical path %q: %w", entry.SourcePath, statErr)
	}
	...
	if readErr != nil {
		return runtimef("rollback: read deprecated %q: %w", entry.DeprecatedPath, readErr)
	}
	restored := stripDeprecationHeader(raw)

	if mkErr := os.MkdirAll(filepath.Dir(entry.SourcePath), 0o750); mkErr != nil {
		return runtimef("rollback: mkdir parent of %q: %w", entry.SourcePath, mkErr)
	}
	if writeErr := os.WriteFile(entry.SourcePath, restored, 0o600); writeErr != nil { //nolint:gosec // audit-supplied canonical path
		return runtimef("rollback: write canonical %q: %w", entry.SourcePath, writeErr)
	}
	if rmErr := os.Remove(entry.DeprecatedPath); rmErr != nil {
		return runtimef("rollback: remove deprecated %q (canonical already restored): %w", entry.DeprecatedPath, rmErr)
	}
	...
	if openErr != nil {
		return runtimef("rollback: open store: %w", openErr)
	}
...
VALUES (?, 'rolled-back', ?, ?, NULL, ?)`
	...
	if _, execErr := st.DB().ExecContext(ctx, insertStmt,
		...
		return runtimef("rollback: insert rolled-back decision: %w", execErr)
	}
