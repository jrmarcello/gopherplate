...
// nudgeTickCandidate mirrors deprecationCandidate but uses JSON tags
...
func runNudgeTick(ctx context.Context, o nudgeTickOpts) error {
	...
	if o.DBPath == "" {
		return usagef("nudge-tick: --db-path must be non-empty")
	}
	if o.Format == "" {
		o.Format = "json"
	}
	if o.Format != "json" {
		return usagef("nudge-tick: unsupported --format %q (only 'json')", o.Format)
	}
	...
	if cfg == nil {
		...
		if loadErr != nil {
			return runtimef("nudge-tick: load config: %w", loadErr)
		}
		cfg = loaded
	}
	...
	if ttl < 1 {
		return usagef("nudge-tick: ttl_days must be >= 1, got %d", ttl)
	}

	st, openErr := openStore(o.DBPath)
	if openErr != nil {
		return runtimef("nudge-tick: open store: %w", openErr)
	}
	...
	if o.Reset {
		if markErr := st.MarkNudgeRun(ctx, now); markErr != nil {
			return runtimef("nudge-tick: mark nudge run: %w", markErr)
		}
		// Empty JSON object signals "reset complete, no payload".
		if _, writeErr := io.WriteString(o.Stdout, "{}\n"); writeErr != nil {
			return runtimef("nudge-tick: write stdout: %w", writeErr)
		}
		return nil
	}
	...
	if listErr != nil {
		return runtimef("nudge-tick: list candidates: %w", listErr)
	}
	...
	for _, r := range rows {
		report.Candidates = append(report.Candidates, nudgeTickCandidate(r))
	}

	enc := json.NewEncoder(o.Stdout)
	enc.SetIndent("", "  ")
	if encErr := enc.Encode(report); encErr != nil {
		return runtimef("nudge-tick: encode json: %w", encErr)
	}
	return nil
}
