...
func TestRunRollback_restoresFileAndAppendsAuditEntry(t *testing.T) {
	...
	entries, readErr := readAudit(learningDirFromDB(dbPath))
	if readErr != nil {
		t.Fatalf("readAudit: %v", readErr)
	}
	if len(entries) != 1 {
	...
	final, _ := readAudit(learningDirFromDB(dbPath))
	if len(final) != 2 {
...
func TestRunRollback_alreadyRolledBackIsUsageError(t *testing.T) {
	...
	if appendErr := appendAudit(learningDir, auditEntry{
		DecisionID:     1,
		Action:         "applied",
