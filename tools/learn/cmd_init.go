...
func runInit(o initOpts) error {
	if o.LearningDir == "" {
		return usagef("init: --dir must be non-empty")
	}
	...
	if mkdirErr := os.MkdirAll(o.LearningDir, 0o700); mkdirErr != nil {
		return runtimef("init: create %q: %w", o.LearningDir, mkdirErr)
	}
	...
	if openErr != nil {
		return runtimef("init: open store: %w", openErr)
	}
	if closeErr := st.Close(); closeErr != nil {
		return runtimef("init: close store: %w", closeErr)
	}
...
// writeDefaultConfigIfAbsent serializes defaultLoopConfig() to YAML and
...
func writeDefaultConfigIfAbsent(path string) error {
	if _, statErr := os.Stat(path); statErr == nil {
		...
		return runtimef("init: stat %q: %w", path, statErr)
	}

	cfg := defaultLoopConfig()
	body, marshalErr := yaml.Marshal(cfg)
	if marshalErr != nil {
		return runtimef("init: marshal default config: %w", marshalErr)
	}
	if len(body) == 0 || body[len(body)-1] != '\n' {
		body = append(body, '\n')
	}
	if writeErr := os.WriteFile(path, body, 0o600); writeErr != nil {
		return runtimef("init: write %q: %w", path, writeErr)
	}
	return nil
}
