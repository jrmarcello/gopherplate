...
// minimalConfig returns a *loopConfig with the supplied min_pattern_freq.
...
func minimalConfig(min int) *loopConfig {
...
func readCandidatesFile(t *testing.T, path string) []*patternCandidate {
	t.Helper()
	...
	out := make([]*patternCandidate, 0, len(lines))
	for i, line := range lines {
		if len(bytes.TrimSpace(line)) == 0 {
			continue
		}
		c, unmarshalErr := unmarshalCandidateStrict(line)
		if unmarshalErr != nil {
