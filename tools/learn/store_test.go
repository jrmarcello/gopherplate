...
func TestOpen_Idempotent(t *testing.T) {
	...
	s1, openErr := openStore(dbPath)
	if openErr != nil {
		t.Fatalf("Open #1: %v", openErr)
	}
	insertErr := s1.InsertEvent(ctx, storeEvent{
...
// TC-UC-31 (REQ-21): corrupted DB file -> Open returns *runtimeError.
func TestOpen_CorruptedDB(t *testing.T) {
	...
	_, openErr := openStore(dbPath)
	if openErr == nil {
		t.Fatalf("expected error on corrupted DB")
	}
	var re *runtimeError
	if !errors.As(openErr, &re) {
		t.Errorf("expected *runtimeError, got %T: %v", openErr, openErr)
	}
}
