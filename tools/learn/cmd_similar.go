...
//     when no tags), wrapped via escapeFTSMatch.
//  5. Call queryFTSIndex(IndexSkill, query, topK, threshold).
//  6. Exclude the target itself from results.
//  7. For each remaining match, fetch the candidate's body from skill_index
//     and compute normalizedLevenshteinDistance for an edit_distance metric.
//  8. Emit JSON array sorted by (-Score, Path) — queryFTSIndex already sorts
...
// queryFn matches the signature of queryFTSIndex. It is injected through
...
type queryFn func(ctx context.Context, db *sql.DB, opts ftsQueryOpts) ([]similarMatch, error)
...
//
// QueryFn defaults to queryFTSIndex; tests inject a fake to control matches.
...
func runSimilar(ctx context.Context, o similarOpts) error {
	...
	if o.SkillPath == "" {
		return usagef("similar: --skill must be non-empty")
	}
	if o.DBPath == "" {
		return usagef("similar: --db-path must be non-empty")
	}
	if o.TopK < 1 {
		o.TopK = defaultSimilarTopK
	}
	if o.QueryFn == nil {
		o.QueryFn = queryFTSIndex
	}
	...
	if threshold == 0.0 {
		...
		if cfg == nil {
			...
			if loadErr != nil {
				return runtimef("similar: load config: %w", loadErr)
			}
			cfg = loaded
		}
	...
	if openErr != nil {
		return openErr // already *runtimeError
	}
	defer func() { _ = st.Close() }()

	matches, queryErr := o.QueryFn(ctx, st.DB(), ftsQueryOpts{
		index:    indexSkill,
		query:    matchExpr,
	...
	if queryErr != nil {
		return runtimef("similar: bm25 query: %w", queryErr)
	}
	...
	for _, m := range matches {
		...
		rows = append(rows, similarRow{
			Path:         m.path,
			Score:        m.score,
			EditDistance: normalizedLevenshteinDistance(target.body, candBody),
		})
		if len(rows) >= o.TopK {
			break
		}
	...
	if marshalErr != nil {
		return runtimef("similar: marshal output: %w", marshalErr)
	}
	body = append(body, '\n')
	if _, writeErr := o.Stdout.Write(body); writeErr != nil {
		return runtimef("similar: write stdout: %w", writeErr)
	}
	return nil
}
...
func readTargetSkill(path string) (targetSkill, error) {
	...
	if readErr != nil {
		if errors.Is(readErr, os.ErrNotExist) {
			return targetSkill{}, usagef("similar: --skill %q: file does not exist", path)
		}
		return targetSkill{}, runtimef("similar: read %q: %w", path, readErr)
	}
	fm, body, splitErr := extractSkillFrontmatter(raw)
	if splitErr != nil {
		return targetSkill{}, usagef("similar: parse %q: %w", path, splitErr)
	}
	return targetSkill{
		path:  path,
		title: fm.Name,
...
func composeMatchQuery(t targetSkill) string {
	...
	for _, p := range parts {
		...
		escaped = append(escaped, escapeFTSMatch(p))
	}
...
func loadSkillBody(ctx context.Context, db *sql.DB, path string) (string, error) {
	...
	if scanErr != nil {
		...
		return "", runtimef("similar: load body %q: %w", path, scanErr)
	}
	return body, nil
}
